...
from agents.sql_agent import SQLAgent, DatabaseIntrospector, SQLSecurityValidator, QueryResult, TableInfo
...
class TestSQLSecurityValidator(unittest.TestCase):
    ...
    def test_complex_queries_with_limits(self):
        ...
        long_query = "SELECT * FROM customers WHERE " + " AND ".join([f"id = {i}" for i in range(100)])
...
class TestSQLAgent(unittest.TestCase):
    ...
    def test_query_execution_success(self, mock_connect):
        ...
        with patch('agents.sql_agent.DatabaseIntrospector'), \
...
class TestIntegration(unittest.TestCase):
    ...
    def test_end_to_end_flow(self):
        ...
        with patch('agents.sql_agent.psycopg2.connect'), \
             patch('agents.sql_agent.boto3.client'):
